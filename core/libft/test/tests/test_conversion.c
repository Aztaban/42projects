...
void run_toupper_test(const char *desc, int c) {
	int std = toupper(c);
	int ft = ft_toupper(c);
	print_result(desc, std == ft);
}

void run_tolower_test(const char *desc, int c) {
	int std = tolower(c);
	int ft = ft_tolower(c);
	print_result(desc, std == ft);
}

void run_atoi_test(const char *desc, const char *input) {
	int std = atoi(input);
...
void	test_atoi(void)
{
	...
	run_atoi_test("ft_atoi(\"   +0424242\")", "   +0424242");
